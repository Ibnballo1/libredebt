/**
 * server/services/admin.service.ts — ENHANCED & SEQUENTIAL-OPTIMIZED FOR max: 1 POOLS
 *
 * All Promise.all calls hitting the database have been flat-mapped to serial await statements.
 * This completely prevents pool exhaustion or timeouts in single-connection environments.
 */
...
export const getSystemOverview = unstable_cache(
  async (): Promise<SystemOverview> => {
    ...
    // Unrolled Promise.all into sequential calls to respect pool limits
    ...
    const newUsers30 = await db
      .select({ value: count() })
      .from(users)
      .where(gte(users.createdAt, thirtyDaysAgo));

    const activeProSubs = await db
      .select({ value: count() })
      .from(subscriptions)
...
export const getSignupGrowth = unstable_cache(
  async (daysBack = 30): Promise<SignupGrowthPoint[]> => {
...
export async function getAdminUserDetail(
  ...
  // Replaced Promise.all with sequential await blocks
...
export const getBusinessMetrics = unstable_cache(
  async (): Promise<BusinessMetrics> => {
    const now = new Date();
    const in7Days = new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000);
    const in30Days = new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000);

    // Replaced Promise.all with sequential awaits
    const totalUsersRes = await db.select({ value: count() }).from(users);

    const totalActiveDebtsRes = await db
      .select({ value: count() })
      .from(debts)
      .where(eq(debts.status, "active"));

    const subsExpiring30 = await db
      .select({ value: count() })
      .from(subscriptions)
      .where(
        and(
    ...
    const receivableBalances = await db
...
export const getEngagementMetrics = unstable_cache(
  async (): Promise<EngagementMetrics> => {
    ...
    // Sequential await blocks instead of simultaneous hits to a small pool
    const totalUsersRes = await db.select({ value: count() }).from(users);
    ...
    const debtsToday = await db
      .select({ value: count() })
      .from(debts)
      .where(gte(debts.createdAt, startOfToday));

    const debtsWeek = await db
      .select({ value: count() })
      .from(debts)
...
export const getStrategyDistribution = unstable_cache(
  async (): Promise<StrategyDistribution> => {
